...
__all__ = ["WORKBOOK_PATH", "REPORT_PATH", "rows", "write_workbook", "render", "write_report"]
...
def rows(gold: goldset.GoldSet | None = None) -> tuple[dict[str, Any], ...]:
    ...
    for concept in concepts:
        ...
        if signed is not None:
            ...
            continue
        # The near-misses are the most useful thing on the row: "deceptively
        # similar is NOT this concept" is what stops a sorter typing by label.
...
def write_workbook(path: Path | None = None, *, generated: str | None = None) -> Path:
    ...
    _mark_as_scoped(book, len(rows()))
    workbook_module.fill(book, {"concept_type": list(rows())})
    book.save(path)
    return path
