...
| 5 | Formalise the ontology | **partial, and unblocked** — nine OWL 2 RL modules drafted in `ontology/draft/`; nothing approved, so `ontology/` stays empty (ADR-0056, ADR-0057). The concept classes now also fill from **authored** `concept_type` records, not only signed ones (ADR-0079 amends ADR-0071) — the 0-of-130 signed taxonomy is an agent's to fill. Whole-Manual scope means the class list stops being 30-of-49 empty for want of material (ADR-0081). **S019 added five concept classes** — `ProcessRole`, `SubjectMatter`, `ProceduralStep`, `InstrumentOrRecord`, `ExternalInstrument` — beside the owner's original four (ADR-0098). **S023 took the relational layer off section 43**: `relations.ttl` was generated from the 35 signed relationships alone, every one sourced to `TMM/Part29`, so the closed predicate list was whatever one Part needed. It is now generated from both registers, with `tmk:ApprovedRelation` (14 terms, unchanged) and `tmk:AuthoredRelation` (7, read by nobody) kept apart (ADR-0109). Two modules stopped explaining their emptiness by naming the withdrawn boundary (ADR-0110) | this repo |
| 6 | Populate and validate the knowledge graph | **partial** — S010, S012, S016. Graph built — as at S023, **53,168 source + 3,243 approved + 8,568 authored triples over 892 chunks**, SHACL gate **0 defects, 0 gaps, 209 notes**, against a *draft* TBox. The source graph tripled because the records now speak about 45 Parts and both instruments rather than one Part (ADR-0097's rule, unchanged; ADR-0109's records). S012: **all files committed** (ADR-0070) and `tmk-graph --check` fails on drift. **S016: a fourth named graph, `graph/authored.ttl`** — built from `authored/` by the same mapping, every node stamped `tmk:origin` and `tmk:reviewStatus`, an authored relationship typed `tmk:AuthoredAssertion` and never `tmk:ApprovedAssertion` (ADR-0091). Empty until something is authored, and the approved count rose by 284 because both stores now stamp their origin | this repo |
...
| Gold-standard dataset | **partial** — 159 approved: 52 concepts (target met), 55 entities, 35 relationships, 10 retrieval questions, 6 reasoning expectations (target met), 1 search question. Beside it and never added to it: **427 authored records, none validated** — 78 concepts, 130 typings, 219 relationships | `eval/gold/`, `authored/` |
...
| Expert input guide | **done, and stale** — ADR-0014. Written for the pre-2026-09-08 model and never revised: its §9 tells a reviewer that a filled-in record without their name is a defect to delete, which now describes all 208 authored records. Banner added S020 naming the three stale parts; a rewrite is owed (Q-60) | `eval/STAGE-0-INPUT-GUIDE.md` |
| Expert request pack | **done** — S020, ADR-0102. Every question for a trade marks expert in one workbook and one plain-English covering note, generated together by `tmk-expert-pack` so their counts cannot disagree. Seven sheets ordered by value: 8 cross-cutting questions, 10 duplicate labels, 9 role words, 5 unjudged modalities, 11 accuracy targets, 78 concepts, 130 typings. The two record sheets keep the intake layout and round-trip through `tmk-transcribe`; the five question sheets return as words | `tmk-expert-pack` → `data/derived/expert-request.xlsx`, `docs/EXPERT-REQUEST.md` |
| Expert review scope | **done** — S020, ADR-0100. What a trade marks expert is asked to look at, in priority order, with readiness stated per item and an equally long list of what is **not** theirs. Two of seven items have a rendered artefact today; three are agent work first. Authored, validated by nobody, and a dated statement rather than a live report | `docs/EXPERT-REVIEW-SCOPE.md` |
...
| Concept typing | **authored, validated by nobody** — S017 typed the 52 signed concepts; S018 authored 78 more from the whole Manual and typed those too, so the sheet carries **130 rows**, every `type` written by a machine and **0 signed by a person**. Each row says whether the *concept* was signed or authored, because a wrong concept is worse than a wrong group. **S019: the taxonomy went from four groups to nine** at the owner's instruction, and the 53 that fitted none of the four were sorted in one pass into `process_role` (8), `subject_matter` (14), `procedural_step` (21), `instrument_or_record` (7) and `external_instrument` (2). One concept stays `none_of_these` deliberately. The five groups and all 53 assignments are an agent's and go back as OQ-0026 (ADR-0092, ADR-0095, ADR-0098) | `authored/concept-types.yaml`, `data/derived/concept-typing.xlsx`, `data/derived/reports/concept-typing.md` |
...
|---|---|---|
| Ontology modules | **drafted, none approved** — 9 modules, 59 classes, OWL 2 RL | `ontology/draft/` |
| Relation dictionary | **generated from both registers** — 18 predicates: 14 approved (35 signed relationships, all `TMM/Part29`) and 7 authored (219 records, **read by nobody**), 4 of them new. Regeneration-checked (ADR-0109) | `ontology/draft/relations.ttl` |
...
| 2 — Vocabulary and knowledge graph | SKOS, ontology modules, relation extraction, provenance, RDF, SHACL, Fuseki | **partial** — SKOS, modules, provenance, RDF and SHACL all exist in draft; the vocabulary covers 39 Parts (ADR-0095) and the relation dictionary covers 45 (ADR-0109). **Relation extraction has started**: `tmk-relationships` is a deterministic pass over the whole corpus and both instruments, 219 records, none reviewed. No triple store is deployed |
