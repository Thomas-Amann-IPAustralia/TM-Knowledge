...
| 2 | Candidate terminology and entities | **open, not started** — gate lifted 2026-09-08 (ADR-0083). Stack fixed (TextRank + YAKE + KeyBERT, spaCy NER as metadata, ADR-0019). **HANDOFF Q3 was answered the same day it started blocking**: Gemini 3.8 Flash, credential in `GEMINI_API_KEY` (ADR-0087). Deterministic extraction stays preferred (rule 7) | this repo |
