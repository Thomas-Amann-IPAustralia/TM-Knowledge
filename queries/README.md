...
**Roadmap Stages 6 and 9.** **Ten demonstration queries — S010.** Generated by
`tmk-graph` from `src/tm_knowledge/graph/queries.py`, where each one carries the
reason it exists. The bar for being here is blunt: if grep over the Manual would do
as well, it does not belong.

Run them all with `tmk-graph --demo`, or read the results with their numbers in
`data/derived/reports/ontology-demonstration.md`.

The two to read first are `sent-elsewhere.rq` — which finds the passage that answers
approved search question GS-0003 and that term frequency ranks 107th of 216 — and
`both-sides.rq`, which catches a record whose subject and object are reversed and
which was approved anyway.

None of them answers a legal question. They report what approved records say, with
the passage and the approver attached.
